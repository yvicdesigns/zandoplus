...
CREATE TABLE IF NOT EXISTS shop_boosts (
  ...
  plan         TEXT NOT NULL DEFAULT 'starter', -- 'starter' | 'premium'
...
ALTER TABLE advertisements
  ...
  ADD COLUMN IF NOT EXISTS ad_type   TEXT DEFAULT 'banner'; -- 'banner'

-- 3. RLS on shop_boosts
ALTER TABLE shop_boosts ENABLE ROW LEVEL SECURITY;

-- Sellers can see and insert their own boosts
CREATE POLICY IF NOT EXISTS "Sellers can read own boosts"
  ON shop_boosts FOR SELECT
  USING (auth.uid() = seller_id);

CREATE POLICY IF NOT EXISTS "Sellers can insert own boosts"
  ON shop_boosts FOR INSERT
  WITH CHECK (auth.uid() = seller_id);

-- Everyone can see active boosts (for FeaturedShopsSection)
CREATE POLICY IF NOT EXISTS "Public can read active boosts"
  ON shop_boosts FOR SELECT
