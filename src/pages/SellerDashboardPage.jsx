      ...
      {editing ? (
        <div className="flex items-center gap-2 flex-1">
          ...
          <button onClick={handleSave} disabled={saving}
            className="w-8 h-8 bg-custom-green-500 text-white rounded-lg flex items-center justify-center flex-shrink-0">
            {saving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Check className="w-3.5 h-3.5" />}
          </button>
          <button onClick={() => { setEditing(false); setVal(value || ''); }}
            className="w-8 h-8 border border-gray-200 text-gray-400 rounded-lg flex items-center justify-center flex-shrink-0">
            <X className="w-3.5 h-3.5" />
          </button>
        </div>
      ) : (
        <>
          <span className="text-[13px] font-semibold text-gray-900 flex-1 px-2 truncate">{value || 'Non renseigné'}</span>
          <button onClick={() => { setEditing(true); setVal(value || ''); }}
            className="w-8 h-8 flex items-center justify-center text-gray-400 hover:text-custom-green-500 rounded-lg transition-colors">
...
const SellerDashboardPage = () => {
  ...
  const load = useCallback(async () => {
    ...
    const grouped = {};
    for (let i = days - 1; i >= 0; i--) grouped[format(subDays(new Date(), i), 'yyyy-MM-dd')] = 0;
  ...
  useEffect(() => {
    if (!user?.id) return;
    const ch = supabase.channel(`seller-sales:${user.id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'transactions_escrow', filter: `vendeur_id=eq.${user.id}` }, () => load())
      .subscribe();
  ...
  /* ── Sections disponibles en navigation mobile ── */
  const MOBILE_SECTIONS = [
    { id: 'produits',     icon: Package,     label: 'Produits',                  badge: null },
    { id: 'commandes',    icon: ShoppingBag, label: 'Commandes',                 badge: stats.pending || null },
    { id: 'avis',         icon: Star,        label: 'Avis clients',              badge: null },
    { id: 'informations', icon: User,        label: 'Informations personnelles', badge: null },
    { id: 'adresses',     icon: MapPin,      label: 'Adresses',                  badge: null },
    { id: 'paiement',     icon: CreditCard,  label: 'Moyens de paiement',       badge: null },
    { id: 'parametres',   icon: Settings,    label: 'Paramètres du compte',      badge: null },
  ];

  const sectionLabel = MOBILE_SECTIONS.find(s => s.id === activeSection)?.label || '';
  ...
  const renderContent = () => {
    ...
    if (activeSection === 'informations') return (
      <div className="bg-white rounded-2xl border border-gray-100 p-4 lg:p-6">
        ...
        <div className="divide-y divide-gray-100">
          ...
          <EditableRow label="Email"             value={user?.email}              onSave={() => toast({ title: 'Contactez le support.' })} />
          <EditableRow label="Téléphone"         value={profile?.phone}           onSave={v => updateField('phone', v)} />
          <EditableRow label="WhatsApp"          value={profile?.whatsapp_number} onSave={v => updateField('whatsapp_number', v)} placeholder="+242 06 000 0000" />
    ...
    if (activeSection === 'paiement') return (
      <div className="bg-white rounded-2xl border border-gray-100 p-6 flex flex-col items-center justify-center py-16 gap-4">
        ...
        <p className="text-[13px] text-gray-400 text-center max-w-xs">La gestion de vos moyens de paiement sera bientôt disponible.</p>
    ...
    if (activeSection === 'parametres') return (
      <div className="bg-white rounded-2xl border border-gray-100 p-4 lg:p-6 space-y-4">
    ...
    return (
      <div className="space-y-5">
        ...
        <div className="rounded-2xl overflow-hidden bg-[#0d1f12]">
          <div className="relative p-5 lg:p-6 flex items-center">
            ...
            {/* Photos produits — desktop only */}
            <div className="absolute right-6 top-6 bottom-6 w-[42%] hidden xl:grid grid-cols-2 gap-2 opacity-50">
            ...
            <div className="relative z-10 flex items-center gap-4">
              <div className="w-16 h-16 lg:w-20 lg:h-20 rounded-full border-4 border-white/30 overflow-hidden bg-white flex items-center justify-center flex-shrink-0 shadow-xl">
                {profile?.avatar_url ? (
                  <img src={profile.avatar_url} alt="" className="w-full h-full object-cover" />
                ) : (
                  <span className="text-xl lg:text-2xl font-black text-custom-green-500">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <h1 className="text-white text-[17px] lg:text-xl font-black">{profile?.full_name || 'Votre boutique'}</h1>
                  {profile?.verified && <BadgeCheck className="w-4 h-4 text-blue-400 flex-shrink-0" />}
                </div>
                <p className="text-green-300 text-[12px] font-semibold mb-2">
                  {profile?.verified ? 'Boutique officielle' : 'Vendeur particulier'}
                </p>
                <div className="flex flex-wrap items-center gap-3 text-[11px]">
                  {memberLabel && (
                    <div className="flex items-center gap-1.5 text-gray-300">
                      <Clock className="w-3 h-3" /><span>{memberLabel}</span>
                    </div>
                  )}
            ...
            {sellerSlug && (
              <Link to={`/seller/${sellerSlug}`}
                className="absolute top-4 right-4 flex items-center gap-1.5 bg-white/10 hover:bg-white/20 text-white text-[11px] font-semibold px-3 py-1.5 rounded-xl border border-white/20 transition-colors backdrop-blur-sm">
                Ma boutique <ExternalLink className="w-3 h-3" />
              </Link>
            )}
          </div>
          {/* Trust badges — 2 cols on mobile, 4 on desktop */}
          <div className="bg-white grid grid-cols-2 lg:grid-cols-4 divide-y lg:divide-y-0 divide-x divide-gray-100 border-t border-gray-100">
            {TRUST.map(({ icon: Icon, title, sub }) => (
              <div key={title} className="flex items-center gap-2.5 px-4 py-3">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4">
          {[
            ...
            { icon: TrendingUp, value: stats.revenue > 0 ? `${stats.revenue.toLocaleString('fr-FR')} F` : '—', label: 'Ventes totales', section: null, linkLabel: 'Bientôt', locked: stats.revenue === 0 },
            { icon: Star,       value: stats.satisfaction != null ? `${stats.satisfaction}%` : '—', label: 'Satisfaction', section: 'avis', linkLabel: 'Voir les avis', locked: stats.satisfaction == null },
          ].map(({ icon: Icon, value, label, section, linkLabel, locked }) => (
            <div key={label} className={`bg-white rounded-2xl border border-gray-100 p-4 ${locked ? 'opacity-50' : ''}`}>
              ...
              <p className="text-xl lg:text-2xl font-black text-gray-900 mb-0.5">{value}</p>
              <p className="text-[11px] text-gray-400 mb-3">{label}</p>
              {locked ? (
                <p className="text-[11px] text-gray-300 flex items-center gap-1"><Lock className="w-3 h-3" /> Bientôt</p>
              ) : section ? (
        ...
        {/* Performances + Commandes récentes — empilé mobile, côte à côte desktop */}
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-5">
          <div className="lg:col-span-7 bg-white rounded-2xl border border-gray-100 p-4 lg:p-5">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-[14px] font-black text-gray-900">
                Performances
                <span className="text-gray-400 font-normal text-[12px] ml-2">({period}j)</span>
              </h3>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
              {[
                ...
                { label: 'Conversion',       value: '—',                            locked: true  },
                ...
                <div key={label} className={locked ? 'opacity-40' : ''}>
                  ...
                  {!locked && <p className="text-[10px] text-emerald-500 font-semibold">réelles</p>}
            ...
            <div className="h-[130px]">
              {chartData.some(d => d.count > 0) ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-gray-300">
                  ...
                  <p className="text-[12px]">Aucune donnée</p>
                </div>
              )}
            </div>
          </div>

          <div className="lg:col-span-5 bg-white rounded-2xl border border-gray-100 p-4 lg:p-5">
            <div className="flex items-center justify-between mb-4">
                  ...
                  return (
                    <div key={o.id} className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-[10px] text-gray-400">#ZND-{shortId}</p>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <span className={`inline-block text-[10px] font-bold px-2 py-0.5 rounded-full mb-0.5 ${cfg.cls}`}>{cfg.label}</span>
                        <p className="text-[11px] font-black text-gray-900">{(o.montant || 0).toLocaleString('fr-FR')} F</p>
                      </div>
                    </div>
                  );
                })}
        ...
        {/* Actions rapides — 2 cols mobile, 5 desktop */}
        <div>
          <h3 className="text-[15px] font-black text-gray-900 mb-3">Actions rapides</h3>
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-3">
            {[
              { icon: Plus,        title: 'Ajouter un produit',  sub: 'Nouveau produit',       onClick: () => navigate('/post-ad'),          locked: false },
              { icon: Package,     title: 'Gérer les produits',  sub: 'Modifier vos produits', onClick: () => setActiveSection('produits'),  locked: false },
              { icon: ShoppingBag, title: 'Commandes',           sub: 'Gérer vos commandes',   onClick: () => setActiveSection('commandes'), locked: false },
              { icon: Megaphone,   title: 'Promouvoir',          sub: 'Visibilité boutique',   onClick: null,                                locked: true  },
              { icon: BarChart2,   title: 'Statistiques',        sub: 'Vos performances',      onClick: null,                                locked: true  },
        ...
        {/* CTA banner */}
        <div className="bg-gradient-to-r from-[#0d1f12] to-custom-green-500 rounded-2xl p-5 flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center flex-shrink-0">
            <Store className="w-7 h-7 text-white" />
          </div>
          <div className="flex-1">
            <h4 className="text-white text-[15px] font-black mb-1">Développez votre activité avec Zando+</h4>
            <p className="text-green-200 text-[12px] leading-relaxed">Outils marketing et audience pour augmenter vos ventes.</p>
          </div>
          <button
            onClick={() => toast({ title: 'Bientôt disponible !' })}
            className="flex-shrink-0 flex items-center gap-2 bg-accent-yellow text-gray-900 font-black text-[12px] px-4 py-2.5 rounded-xl hover:opacity-90 transition-opacity whitespace-nowrap"
          >
  ...
  return (
    <>
      ...
      {/* ══════════════════════════════════════════
          MOBILE — drill-down iOS style
          ══════════════════════════════════════════ */}
      <div className="lg:hidden bg-page-bg min-h-screen">

        {activeSection === 'dashboard' ? (
          /* ── Vue accueil mobile ── */
          <div>
            {/* Header vert vendeur */}
            <div className="bg-[#0d1f12] px-4 pt-5 pb-6">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-16 h-16 rounded-full border-4 border-white/30 overflow-hidden bg-white flex items-center justify-center flex-shrink-0 shadow-xl">
                  {profile?.avatar_url
                    ? <img src={profile.avatar_url} alt="" className="w-full h-full object-cover" />
                    : <span className="text-xl font-black text-custom-green-500">{(profile?.full_name || 'V').charAt(0).toUpperCase()}</span>
                  }
                </div>
                <div>
                  <div className="flex items-center gap-1.5 mb-0.5">
                    <p className="text-white text-[17px] font-black">{profile?.full_name || 'Votre boutique'}</p>
                    {profile?.verified && <BadgeCheck className="w-4 h-4 text-blue-400" />}
                  </div>
                  <p className="text-green-300 text-[12px] font-semibold">{profile?.verified ? 'Boutique officielle' : 'Vendeur particulier'}</p>
                  {memberLabel && <p className="text-gray-400 text-[11px] mt-0.5">{memberLabel}</p>}
                </div>
              </div>
              {sellerSlug && (
                <Link to={`/seller/${sellerSlug}`}
                  className="inline-flex items-center gap-1.5 bg-white/10 text-white text-[12px] font-semibold px-4 py-2 rounded-xl border border-white/20 transition-colors">
            ...
            <div className="grid grid-cols-2 gap-3 px-4 -mt-3">
              {[
                { icon: Package,    value: stats.listings, label: 'Produits',    section: 'produits'  },
                { icon: ShoppingBag,value: stats.orders,   label: 'Commandes',   section: 'commandes' },
                { icon: TrendingUp, value: stats.revenue > 0 ? `${(stats.revenue/1000).toFixed(0)}k F` : '—', label: 'Revenus', section: null },
                { icon: Star,       value: stats.satisfaction != null ? `${stats.satisfaction}%` : '—', label: 'Satisfaction', section: 'avis' },
              ].map(({ icon: Icon, value, label, section }) => (
                <button key={label}
                  onClick={() => section && setActiveSection(section)}
                  className={`bg-white rounded-2xl border border-gray-100 p-4 text-left shadow-sm ${section ? 'active:scale-95 transition-transform' : 'cursor-default'}`}
                >
                  <Icon className="w-4 h-4 text-custom-green-500 mb-2" />
                  <p className="text-xl font-black text-gray-900">{typeof value === 'number' ? value.toLocaleString('fr-FR') : value}</p>
            ...
            <div className="px-4 mt-5">
              <p className="text-[13px] font-black text-gray-700 mb-3">Actions rapides</p>
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => navigate('/post-ad')}
                  className="bg-custom-green-500 rounded-2xl p-4 flex flex-col items-start gap-2 active:opacity-80">
                  <Plus className="w-5 h-5 text-white" />
                  <p className="text-[13px] font-bold text-white">Ajouter un produit</p>
                </button>
                <button onClick={() => setActiveSection('commandes')}
                  className="bg-white rounded-2xl border border-gray-100 p-4 flex flex-col items-start gap-2 active:bg-gray-50">
                  <ShoppingBag className="w-5 h-5 text-custom-green-500" />
                  <div className="flex items-center gap-2">
                    <p className="text-[13px] font-bold text-gray-900">Commandes</p>
                    {stats.pending > 0 && (
                      <span className="w-5 h-5 bg-orange-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{stats.pending}</span>
                    )}
                  </div>
                </button>
              </div>
            ...
            <div className="px-4 mt-5 mb-6">
              <p className="text-[13px] font-black text-gray-700 mb-3">Gestion de la boutique</p>
              <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden divide-y divide-gray-100">
                {MOBILE_SECTIONS.map(({ id, icon: Icon, label, badge }) => (
                  <button key={id} onClick={() => setActiveSection(id)}
                    ...
                    <span className="flex-1 text-[14px] font-semibold text-gray-900">{label}</span>
                    {badge != null && (
                      <span className="w-5 h-5 bg-orange-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{badge}</span>
                    )}
          ...
          <div className="min-h-screen">
            ...
            <div className="bg-white border-b border-gray-100 sticky top-[60px] z-10">
              <button
                ...
                className="flex items-center gap-2 px-4 py-3.5 text-custom-green-600 font-semibold text-[14px]"
              >
      ...
      {/* ══════════════════════════════════════════
          DESKTOP — sidebar + main côte à côte
          ══════════════════════════════════════════ */}
      <div className="hidden lg:block bg-page-bg min-h-screen">
        {/* Fil d'Ariane */}
          ...
          <div className="grid grid-cols-12 gap-6">
            ...
            <aside className="col-span-3">
              ...
              <div className="bg-white rounded-2xl border border-gray-100 p-3 space-y-1">
                <NavItem icon={LayoutDashboard} label="Tableau de bord" active={activeSection === 'dashboard'} onClick={() => setActiveSection('dashboard')} />
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-3 pt-3 pb-1">Gestion boutique</p>
                <NavItem icon={Store}       label="Ma boutique"          to={sellerSlug ? `/seller/${sellerSlug}` : '#'} />
                <NavItem icon={Package}     label="Produits"             active={activeSection === 'produits'} onClick={() => setActiveSection('produits')} />
                <NavItem icon={ShoppingBag} label="Commandes"            active={activeSection === 'commandes'} onClick={() => setActiveSection('commandes')} badge={stats.pending || undefined} />
                <NavItem icon={RotateCcw}   label="Retours"              locked />
                <NavItem icon={Star}        label="Avis clients"         active={activeSection === 'avis'} onClick={() => setActiveSection('avis')} />
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-3 pt-3 pb-1">Marketing</p>
                ...
                <NavItem icon={User}       label="Informations"          active={activeSection === 'informations'} onClick={() => setActiveSection('informations')} />
                <NavItem icon={CreditCard} label="Moyens de paiement"   active={activeSection === 'paiement'} onClick={() => setActiveSection('paiement')} />
                <NavItem icon={MapPin}     label="Adresses"              active={activeSection === 'adresses'} onClick={() => setActiveSection('adresses')} />
                <NavItem icon={Settings}   label="Paramètres"            active={activeSection === 'parametres'} onClick={() => setActiveSection('parametres')} />
              </div>
              <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-100 rounded-2xl p-4 mt-4 text-center">
                ...
                <p className="text-[11px] text-gray-500 mb-3">Notre équipe est disponible pour vous.</p>
