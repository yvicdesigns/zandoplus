...
const AdminEscrowTab = memo(() => {
  ...
  const handleSendWhatsApp = async () => {
    ...
    let proofLine = '';
    if (messageTarget.preuve_paiement_url) {
      try {
        const urlObj = new URL(messageTarget.preuve_paiement_url);
        const filePath = urlObj.pathname.split('/payment_proofs/')[1];
        if (filePath) {
          const { data, error } = await supabase.storage
            .from('payment_proofs')
            .createSignedUrl(filePath, 259200); // 3 jours
          if (!error && data?.signedUrl) {
            proofLine = `\n\n📎 Preuve de paiement envoyée : ${data.signedUrl}`;
          }
        }
      } catch (_) { /* on envoie sans le lien si ça échoue */ }
    }

    const base = messageContent.trim() ||
      `Bonjour ${messageTarget.acheteur?.full_name || ''}, nous avons besoin de clarifier votre commande *${messageTarget.annonce?.title || ''}* sur Zando+.`;
    const text = encodeURIComponent(`${base}${proofLine}`);
                ...
                <div key={tx.id} className="bg-white rounded-xl border border-orange-200 p-4 space-y-3">
                  ...
                  <div className="bg-orange-50 border border-orange-200 rounded-lg px-4 py-3 space-y-1">
                    ...
                    <p className="text-xs text-gray-500">
                      (Reçu {tx.montant.toLocaleString('fr-FR')} FCFA — commission {commission.toLocaleString('fr-FR')} FCFA)
                    </p>
            ...
            <DialogDescription>
              ...
              <strong>
                {completeWithdrawTarget
                  ? (completeWithdrawTarget.montant - (completeWithdrawTarget.commission_amount ?? computeCommission(completeWithdrawTarget.montant))).toLocaleString('fr-FR')
                  : 0
                } FCFA
              </strong>{' '}
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              Montant net : <strong>{(releaseTarget?.montant - (releaseTarget?.commission_amount ?? computeCommission(releaseTarget?.montant)))?.toLocaleString('fr-FR')} FCFA</strong>.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
        ...
        <DialogContent>
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            <Button
              ...
              disabled={actionLoading?.id === messageTarget?.id}
            >
              <WhatsAppIconInline /> Envoyer via WhatsApp
            </Button>
            <Button
