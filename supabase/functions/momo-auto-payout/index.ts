...
serve(async (req) => {
  ...
  try {
    ...
    for (const tx of batch || []) {
      const fallbackCommission = tx.montant < 10000
        ? Math.round(tx.montant * 0.08)
        : tx.montant <= 200000
          ? Math.round(tx.montant * 0.05)
          : 10000;
      const net = tx.montant - (tx.commission_amount ?? fallbackCommission);
